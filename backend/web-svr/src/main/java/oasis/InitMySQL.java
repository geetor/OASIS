...
import oasis.vo.entity.AuthorSearchedVO;
...
//返回结果
    ...
    public ArrayList<Integer> insertAffiliAndAuthor(CsvReader cr, Connection conn, ArrayList<Integer> authorIdList, ArrayList<Integer> affilicationIdList) {
        ...
        try {
            ...
            String[] authorAbbNameList=cr.get(3).split(";");
            ...
            for (int i = 0; i < affilicationList.length; i++) {
                if (affilicationList[i] != "" && i < authorList.length) {
                    ...
                    sql = "select affiliation_id from affiliation_info where affiliation_name=\"" + affilicationList[i].trim().replace("\"","\'") + "\"";
                    rs = stmt.executeQuery(sql);
                    if (rs.next() == false) {
                        //如果不存在机构信息，则插入
                        stmt = conn.createStatement();
                        sql = "insert into affiliation_info (affiliation_name) values(\"" + affilicationList[i].trim().replace("\"","\'") + "\")";
                        stmt.execute(sql);
                        //获取机构id
                        stmt = conn.createStatement();
                        sql = "select affiliation_id from affiliation_info where affiliation_name=\"" + affilicationList[i].trim().replace("\"","\'") + "\"";
                    ...
                    if (rs.next() == false) {
                        stmt = conn.createStatement();
                        sql = "insert into author_info (author_name,affiliation_id,author_abb_name) values (\"" + authorList[i].trim() + "\"," + affiliationId + ",\""+authorAbbNameList[i].trim()+"\")";
                        stmt.execute(sql);
                        stmt = conn.createStatement();
                        sql = "select author_id from author_info where author_name=\"" + authorList[i].trim() + "\"" + "and affiliation_id=" + affiliationId;
                        rs = stmt.executeQuery(sql);
    ...
    public ArrayList<Integer> insertKeyword(CsvReader cr, Connection conn, ArrayList<Integer> keyWordIdList) {
        ...
        try {
            ...
            String[] keywordsList = cr.get(11).split(";");
    ...
    public int insertPublication(CsvReader cr, Connection conn, String type) {
        ...
        int publicationId = -1;
        try {
            String publicationTitle = cr.get(4).trim();
    ...
    public int insertAtricle(CsvReader cr, Connection conn, int publicationId) {
        ...
        try {
            ...
            int startPage = 0;
            if (cr.get(6).trim().length() != 0) {
                startPage = Integer.parseInt(cr.get(6).trim());
            }
            int endPage = 0;
            if (cr.get(7).trim().length() != 0) {
                endPage = Integer.parseInt(cr.get(7).trim());
            }
            String docAbstract = cr.get(8).trim().replace("\"", "'");
            String doi = cr.get(9).trim();
            String pdfLink = cr.get(10).trim();
            int referenceCount = 0;
            if (cr.get(13).trim().length() != 0) {
                referenceCount = Integer.parseInt(cr.get(13).trim());
            }
            int citation = 0;
            if (cr.get(12).trim().length() != 0) {
                citation = Integer.parseInt(cr.get(12).trim());
            }
            String publisher = cr.get(14).trim();
            String docIdentifier = cr.get(15).trim();
    ...
    //初始化Icse的hindex指标
    public void initHindexOfPublication(Connection con,String type) {
        Statement stmt = null;
        String sql = null;
        ResultSet rs = null;
        Statement stmt2 = null;
        ...
        try {
            stmt = con.createStatement();
            sql = "select publication_year,publication_id from publication_info where publication_type=\""+type+"\"";
            rs = stmt.executeQuery(sql);
            int pbYear = 0;
            int pbId = 0;
            int pbHindex = 0;
            while (rs.next()) {
                pbId = rs.getInt("publication_id");
                pbYear = Integer.parseInt(rs.getString("publication_year"));
                pbHindex = calHindexOfPublication(pbYear, con,type);
    ...
    public void initRelatedAuthors2(Connection conn,String path){
        try{
            ...
            for (Integer authorId : authorIds) {
                ...
                String authorSql2 = "select related_authors from author_info where author_id="+authorId;
                ResultSet authorSet2 = stmt2.executeQuery(authorSql2);
                if (authorSet2.next()){
                    String auName=authorSet2.getString("related_authors");
                    if (auName!=null&&auName.length()!=0){
                        continue;
                    }else{
                        Integer newId=authorId%4000;
                        Statement stmt3 = conn.createStatement();
                        String authorSql3 = "select related_authors from author_info where author_id="+newId;
                        ResultSet authorSet3 = stmt3.executeQuery(authorSql3);
                        if (authorSet3.next()){
                            String relatedAuthors2=authorSet3.getString("related_authors");
                            String insertSql = "update author_info set related_authors = \"" + relatedAuthors2 + "\" where author_id = " + authorId;
    ...
    public void initRelatedAuthors(Connection conn, String path) {
        try {
            ...
            for (Integer authorId : authorIds) {
                ...
                strNeighbors = "'" + strNeighbors.replace(", ", "*").replace("[", "").replace("]", "") + "'";
    ...
    public int calHindexOfPublication(int year, Connection con,String type) {
        Statement stmt = null;
        String sql = null;
        ResultSet rs = null;
        ArrayList<Integer> referenceList = new ArrayList<>();
        ...
        try {
            int pbId = 0;
            stmt = con.createStatement();
            sql = "select publication_id from publication_info where publication_year=\"" + year + "\" and publication_type=\""+type+"\"";
            rs = stmt.executeQuery(sql);
            if (rs.next()) {
                pbId = rs.getInt("publication_id");
            }
    ...
    public void insertCitationArticle(CsvReader cr,Connection conn,int articleId){
        ...
        try {
            ...
            String[] citationArticleNameList = cr.get(16).split(";");
            for (int i = 0; i < citationArticleNameList.length; i++) {
                if (citationArticleNameList[i]==null||citationArticleNameList[i].length()==0)continue;

                stmt = conn.createStatement();
                sql = "insert into citation_info (doc_id, citation_article_name) values("+articleId+" ,\"" + citationArticleNameList[i].trim().replace("\"","\'") + "\")";
                stmt.execute(sql);
            }
    ...
    public void insertReferenceArticle(CsvReader cr,Connection conn,int articleId){
        ...
        try {
            ...
            String[] referenceArticleNameList = cr.get(17).replace("\"","\'").split(";");
            for (int i = 0; i < referenceArticleNameList.length; i++) {
                if (referenceArticleNameList[i]==null||referenceArticleNameList[i].length()==0)continue;
                stmt = conn.createStatement();
                sql = "insert into reference_info (doc_id,reference_article_name ) values("+articleId+" ,\"" + referenceArticleNameList[i].trim().replace("\"","\'") + "\")";
                stmt.execute(sql);
            }
    ...
    //读取csv文件并初始化数据库
    public void readFromCsv(Connection conn, String path, String publicationType,String localPath) {
...
//            InputStream in=new FileInputStream(new File(localPath));
            ...
            while (cr.readRecord()) {
                ...
                insertCitationArticle(cr,conn,articleId);
                insertReferenceArticle(cr,conn,articleId);
    ...
    public void initLinkedAuthors(Connection conn) {
        try {
            ...
            int[][] records = new int[11425][11425];
...
//            ConnectOSS connectOSS=new ConnectOSS();
//            List<String> existFileNameList=new ArrayList<>();
//            Statement stmt = conn.createStatement();
//
//            String sql = "select distinct csv_name from source_csv";
//            ResultSet resSet = stmt.executeQuery(sql);
//            while (resSet.next()) {
//                existFileNameList.add(resSet.getString("csv_name"));
//            }
//
//            String preLocalPath="e://ossFile/";
//            String s2 = "icse_ieee_09-19.csv";
//            String s3="ase_ieee_09-19.csv";
//            System.out.println("开始初始化连接");
//            readFromCsv(conn, "datasource/ase13_15_16_17_19.csv", "icse",preLocalPath+s2);
//            readFromCsv(conn, "datasource/ase13_15_16_17_19.csv", "ase",preLocalPath+s3);
//            //此处待修改

//            for (String fileName:existFileNameList){
//                if (connectOSS.writeToLocal(fileName,preLocalPath+fileName)){
//                    readFromCsv(conn, "datasource/ase13_15_16_17_19.csv", "ase",preLocalPath+fileName);
//                    System.out.println(fileName+"数据初始化完毕");
//                }else {
...
//            }
            ...
            readFromCsv(conn, "datasource/icse_ieee_10-19.csv", "icse","");
            System.out.println(1);
            readFromCsv(conn, "datasource/ase_ieee_10-19.csv", "ase","");
            System.out.println(2);
            readFromCsv(conn, "datasource/tse_ieee_10-19.csv", "tse","");
            ...
            initHindexOfPublication(conn,"tse");
            System.out.println("tse的hindex初始化完毕");
...
//            initDataModel(conn);
//            initRelatedAuthors2(conn, "datasource/model.csv");
